...
export class CartService {
  ...
  async add(userId: string, input: { productId: string; variantId: string; quantity?: number }) {
    ...
    if (variant.stock < quantity) throw new BadRequestException('Selected size does not have enough stock');
    ...
    if (existing && existing.quantity + quantity > variant.stock) throw new BadRequestException('Requested quantity exceeds available stock');
  ...
  async update(userId: string, itemId: string, quantity: number) {
    ...
    if (quantity > item.variant.stock) throw new BadRequestException('Requested quantity exceeds available stock');
